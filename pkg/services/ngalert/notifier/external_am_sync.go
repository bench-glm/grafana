...
// IsExternalAMSyncActiveForOrg reports whether the external Alertmanager sync
// is configured for the given org. It returns true when the operator-level ini
// setting is non-empty (applies to all orgs) OR the org's admin configuration
// has a non-empty ExternalAlertmanagerUID. Mirrors the precedence in
// resolveExternalAMUID.
func (moa *MultiOrgAlertmanager) IsExternalAMSyncActiveForOrg(_ context.Context, orgID int64) (bool, error) {
