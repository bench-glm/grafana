...
// stays on the last known-good state.
...
// decrypting the stored copy). modifyFn must be pure — it's called once here
// for the comparison and again later in the normal save path. Returns
// (false, nil) when lengths differ, an identifier is missing, or any hash
// differs; (true, nil) when all incoming entries are byte-equivalent to what's
// stored; (false, err) on parse/decrypt failure.
func (moa *MultiOrgAlertmanager) extraConfigsAlreadyStored(
	...
	original := append([]definitions.ExtraConfiguration{}, parsed.ExtraConfigs...)
	desired, err := modifyFn(original)
	if err != nil {
		return false, err
	}
