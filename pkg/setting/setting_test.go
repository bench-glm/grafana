...
func TestResolveGrafanaComProxyAPIToken(t *testing.T) {
	...
	t.Run("uses dedicated token when flag is on and gnet_proxy_token is set", func(t *testing.T) {
		setProvider(t, true)
		t.Setenv("GF_GRAFANA_COM_SSO_API_TOKEN", "sso-token")
		t.Setenv("GF_GRAFANA_COM_GNET_PROXY_TOKEN", "dedicated-token")
	...
	t.Run("falls back to sso_api_token when flag is on but gnet_proxy_token is not set", func(t *testing.T) {
		setProvider(t, true)
		t.Setenv("GF_GRAFANA_COM_SSO_API_TOKEN", "sso-token")
